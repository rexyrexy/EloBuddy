...
namespace SPrediction
{
    ...
    public static class Prediction
    {
        ...
        public struct Result
        {
            ...
            {
                if (this.Input.RangeCheckFrom.To2D().LSDistance(this.CastPosition) > this.Input.SpellRange - (checkDodge ? GetArrivalTime(this.Input.From.To2D().LSDistance(this.CastPosition), this.Input.SpellDelay, this.Input.SpellMissileSpeed) * this.Unit.MoveSpeed * (100 - ConfigMenu.MaxRangeIgnore) / 100f : 0))
        ...
        public static EloBuddy.SDK.Menu.Menu m;
        ...
        public static void Initialize()
        {
            ...
            {
                m = MainMenu.AddMenu("SPrediction", "SPREDX");
                m.AddLabel("0 => SPredction || 2 => Common Prediction");
                m.Add("PREDICTIONLIST", new EloBuddy.SDK.Menu.Values.Slider("Prediction Method", 1, 0, 1));
            }
        }
        ...
        internal static Result GetPrediction(Obj_AI_Base target, float width, float delay, float missileSpeed, float range, bool collisionable, SkillshotType type, List<Vector2> path, float avgt, float movt, float avgp, float anglediff, Vector2 from, Vector2 rangeCheckFrom)
        {
            ...
            try
            {
                ...
                if (target is AIHeroClient)
                {
                    ...
                    {
                        if (target.AttackCastDelay * 1000 + PathTracker.EnemyInfo[target.NetworkId].AvgOrbwalkTime + avgt - width / 2f / target.MoveSpeed >= GetArrivalTime(target.ServerPosition.To2D().LSDistance(from), delay, missileSpeed))
                        {
                ...
                float d = result.CastPosition.LSDistance(target.ServerPosition.To2D());
