...
namespace SPrediction
{
    ...
    public static class VectorPrediction
    {
        ...
        /// <returns>Prediction result as <see cref="Vector.Result"/></returns>
        ...
        public static AoeResult GetAoePrediction(float width, float delay, float vectorSpeed, float range, float vectorLenght, Vector2 rangeCheckFrom)
        {
            ...
            var enemies = HeroManager.Enemies.Where(p => p.IsValidTarget() && Prediction.GetFastUnitPosition(p, delay, 0, rangeCheckFrom).LSDistance(rangeCheckFrom) < range);
